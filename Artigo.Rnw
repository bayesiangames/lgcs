...
Originalmente estabelecida em \citeyear{Cournot.1838} por \citeauthoronline{Cournot.1838} a teoria alçou proeminência a partir do livro \emph{The Theory of Games and Economic Behavior} de \citeonline{Neumann.1947} que formalizou modelos matemáticos para estudo do comportamento econômico, nas interações entre agentes, em cenários de conflito ou cooperação. Transcendendo o ramo econômico, a teoria dos jogos ganhou aplicações em diversas áreas como: militar \cite{Haywood.1954,RAND.2004}, jurídica \cite{Rosa.2014}, biológica \cite{Smith.1982}, filosófica \cite{Lewis.2002} e política \cite{Levy.2003}. 
...
O presente artigo é uma abordagem sobre a execução do modelo \emph{Shapley Value} para equacionar a contribuição marginal entre agentes de uma CS. A precípua está no rateio do custo rodoviário itinerário --- comensurado em Km. Considerando uma hipotética coalizão formada por três fornecedores, adiciona-se indivíduos, um de cada vez, dando a cada um uma contribuição marginal $v(S \cup \{i\})$ --- $v(S)$ para $S$ indivíduos. Na medida em que os custos são ponderados atinge-se uma noção de justiça. A \autoref{sec:caso} estrutura a narrativa do caso de estudo. Na \autoref{sec:teoria} estudam-se os conceitos da teoria, na ambiência do cenário fictício. A \autoref{sec:R} implementa o algorítmo na linguagem R\footnote{\citetext{R.2016}} --- viabilizando futuras simulações. Ademais, a \autoref{sec:conclusao} retoma a proposta adicionando um parecer de conclusão; e, em fim, a \autoref{sec:futuro} projetar novas lacunas com enlace para investigações em trabalhos futuros.
