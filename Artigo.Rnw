...
citation(package = "GameTheory")


ggmap(get_map(location = c(lon = -51.01476, lat = -29.64444), 
              zoom = 16,
              maptype = c("terrain"),
              language = "pt-BR"))
...
<<echo=TRUE, warning=FALSE, results='markup', message=FALSE, highlight=TRUE, error=FALSE, fig.show='hold', fig.align='center', fig.cap="Itinerário para rateio de custo">>=

ggmap(get_map("Sao Paulo, SP, Brasil", 
              zoom = 5,
              maptype = c("terrain"),
              language = "pt-BR"))
